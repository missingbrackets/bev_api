...
from algorithms.tasks_section.adverse_weather import bev_task_batches_threaded
...
def adverse_weather_rating(hxd, progress):
    ...
    api_key = hx.secrets.bev_api_key
    ...
    df_event_info['bev_location'] =   df_event_info['location/country'] + ", " + df_event_info['location/area'] + ", " + df_event_info['location/city']
    ...
    daily_api_json = bev_task_batches_threaded(api_key=api_key, perils=perils["daily"], endpoint="daily", event_set=daily_event_set)

    df_daily = (
        pd.DataFrame(daily_api_json)
        .set_index(['index', 'peril'])['model']
        .apply(pd.Series)
        .stack(dropna=True)
        .reset_index()
        .rename(columns={'level_2': 'threshold', 0: 'value'})
        .sort_values('index')
        )
    ...
    expanding_event_set=(
        df_event_set.loc[df_event_set["requires_cumulative"] == 1, ["index", "location", "start_date", "end_date"]]
        .to_dict(orient="records")
    )
    ...
    if len(expanding_event_set) > 0:
        ...
        expanding_api_json = bev_task_batches_threaded(api_key=api_key, perils=perils["expanding"], window_days=expanding_window, endpoint="expanding", event_set=expanding_event_set)

        df_expanding = (
            pd.DataFrame(expanding_api_json)
            .set_index(['index', "window_index", 'peril'])['model']
            .apply(pd.Series)
            .stack(dropna=True)
            .reset_index()
            .rename(columns={'level_3': 'threshold', 0: 'value'})
            )
